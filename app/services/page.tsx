    ...
    <>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 24 }}>
          {services.map((s, i) => (
            <Reveal key={i} delay={i * 100} className="service-card">
              ...
              <p style={{ color: "#777", fontSize: "0.9rem", lineHeight: 1.7, marginBottom: 20 }}>{s.desc}</p>
              <ul style={{ listStyle: "none" }}>
                {s.features.map((f, j) => (
                  <li key={j} style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 10, color: "#AAA", fontSize: "0.85rem", lineHeight: 1.5 }}>
      ...
      <section style={{ padding: "100px 5%", background: "#0D0D0D" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <Reveal style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "Playfair Display, serif", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 700, marginBottom: 16 }}>The Picture It <span className="gold-gradient">Difference</span></h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 24 }}>
            {whyChooseUs.map((w, i) => (
              <Reveal key={i} delay={i * 100} className="service-card" style={{ textAlign: "center" }}>
                ...
                <h3 style={{ fontFamily: "Playfair Display, serif", fontSize: "1.05rem", fontWeight: 600, marginBottom: 12 }}>{w.title}</h3>
                <p style={{ color: "#777", fontSize: "0.85rem", lineHeight: 1.7 }}>{w.desc}</p>
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "100px 5%", background: "#0A0A0A" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 80, alignItems: "center" }} className="about-grid">
          <Reveal>
            <img src="/homeabout.png" alt="Event guests" style={{ width: "100%", height: 460, objectFit: "cover", borderRadius: 8, border: "1px solid #2A2A2A" }} />
          </Reveal>
          <Reveal delay={150}>
            <p style={{ color: "#C9A84C", fontSize: "0.75rem", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 12 }}>Every Detail, Handled</p>
            <h2 style={{ fontFamily: "Playfair Display, serif", fontSize: "clamp(1.8rem, 3vw, 2.5rem)", fontWeight: 700, marginBottom: 24, lineHeight: 1.2 }}>A Centerpiece, Not Just a <span className="gold-gradient">Booth</span></h2>
            <p style={{ color: "#888", lineHeight: 1.9, marginBottom: 20 }}>Whatever your celebration calls for — a sleek digital photobooth, a cinematic 360 video experience, or a fully branded corporate activation — our team designs every detail around your event.</p>
            <p style={{ color: "#888", lineHeight: 1.9, marginBottom: 36 }}>From custom overlays and premium props to instant sharing and a dedicated attendant, we make sure your guests are entertained and your memories are captured in style.</p>
      ...
      <section style={{ padding: "80px 5%", background: "linear-gradient(135deg, #0A0A0A 0%, #1A1200 50%, #0A0A0A 100%)", borderTop: "1px solid #2A1A00", borderBottom: "1px solid #2A1A00" }}>
        <Reveal style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "Playfair Display, serif", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 700, marginBottom: 20 }}>Let&#39;s Plan Your <span className="gold-gradient">Next Event</span></h2>
          <p style={{ color: "#888", maxWidth: 500, margin: "0 auto 40px" }}>Tell us about your celebration and we&#39;ll help you choose the perfect experience.</p>
