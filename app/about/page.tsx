    ...
    <>
      ...
      <section style={{ padding: "40px 5% 100px", background: "#0A0A0A" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 80, alignItems: "center" }} className="about-grid">
          <Reveal style={{ position: "relative" }}>
            <img src="/homeimg.png" alt="About" style={{ width: "100%", height: 480, objectFit: "cover", borderRadius: 8 }} />
          ...
          <Reveal delay={150}>
            <p style={{ color: "#888", lineHeight: 1.9, marginBottom: 20 }}>Founded with a vision to revolutionize the event photography industry, Picture It Productions blends cutting-edge technology with high-end luxury service. We believe every event tells a story.</p>
            <p style={{ color: "#888", lineHeight: 1.9, marginBottom: 36 }}>From intimate weddings to massive corporate galas, we bring the same level of meticulous detail and professional energy to every booking. We are not just providing a booth — we are providing a centerpiece of entertainment.</p>
            <div style={{ display: "flex", gap: 48, marginBottom: 36, flexWrap: "wrap" }}>
              {[{ num: "500+", label: "Luxury Events" }, { num: "5 Star", label: "Rated" }, { num: "4+", label: "Years Experience" }, { num: "GTA", label: "Toronto & Area" }].map(s => (
                <div key={s.num}>
                  ...
                  <div style={{ color: "#666", fontSize: "0.7rem", letterSpacing: "0.1em", textTransform: "uppercase", marginTop: 4 }}>{s.label}</div>
                </div>
              ))}
            </div>
      ...
      <section style={{ padding: "100px 5%", background: "#0D0D0D" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <Reveal style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "Playfair Display, serif", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 700, marginBottom: 16 }}>Our Core <span className="gold-gradient">Values</span></h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 24 }}>
            {values.map((v, i) => (
              <Reveal key={i} delay={i * 100} className="service-card" style={{ textAlign: "center" }}>
                ...
                <h3 style={{ fontFamily: "Playfair Display, serif", fontSize: "1.1rem", fontWeight: 600, marginBottom: 12 }}>{v.title}</h3>
                <p style={{ color: "#777", fontSize: "0.85rem", lineHeight: 1.7 }}>{v.desc}</p>
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "100px 5%", background: "#0A0A0A" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <Reveal style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "Playfair Display, serif", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 700, marginBottom: 16 }}>Milestones Along the <span className="gold-gradient">Way</span></h2>
            <div className="section-divider" />
          </Reveal>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 32 }}>
            {milestones.map((m, i) => (
              <Reveal key={i} delay={i * 100} style={{ textAlign: "center" }}>
                ...
                <h3 style={{ fontFamily: "Playfair Display, serif", fontSize: "1.2rem", fontWeight: 700, marginBottom: 12 }}>{m.title}</h3>
                <p style={{ color: "#777", fontSize: "0.875rem", lineHeight: 1.7 }}>{m.desc}</p>
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "80px 5%", background: "linear-gradient(135deg, #0A0A0A 0%, #1A1200 50%, #0A0A0A 100%)", borderTop: "1px solid #2A1A00", borderBottom: "1px solid #2A1A00" }}>
        <Reveal style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "Playfair Display, serif", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 700, marginBottom: 20 }}>Let&#39;s Create Your <span className="gold-gradient">Next Memory</span></h2>
          <p style={{ color: "#888", maxWidth: 500, margin: "0 auto 40px" }}>Become part of the 500+ luxury events we&#39;ve had the honor of capturing.</p>
